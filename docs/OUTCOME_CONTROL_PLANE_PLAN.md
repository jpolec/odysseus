...
| Routing | 0.8 shadow router, evidence/sample floor, backtest, export and drift | Yes | Lane-level, limited objectives and no policy profiles/calibration | Add RoutingDecision v2 after the ledger is trustworthy | P2 |
| Confidence | Evaluation engine emits score/confidence | Partial | Precise score is not an empirical delivery probability | Rename heuristic signal and add calibration datasets/curves | P2 |
